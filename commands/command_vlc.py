...
class CommandVlc(commands.Component):
    def __init__(self) -> None:
        if os.environ["COMMAND_VLC_ADDRESS"] == "":
            LOGGER.error("no valid vlc address provided in env vars")

        else:
            self.vlc_address = os.environ["COMMAND_VLC_ADDRESS"]
            self.vlc_auth = (os.environ["COMMAND_VLC_LOGIN"], os.environ["COMMAND_VLC_PASS"])
    ...
    def get_status(self) -> bool:
        req = requests.get(self.get_status_url(), auth=self.vlc_auth)
    ...
    async def nextsong(self, ctx: commands.Context) -> None:
        ...
        req = requests.get(self.get_next_url(), auth=self.vlc_auth)
        if req.status_code != 200:
